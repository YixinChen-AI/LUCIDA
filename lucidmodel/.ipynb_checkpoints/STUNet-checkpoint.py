...
class BasicResBlock(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size=3, padding=1, stride=1, use_1x1conv=False):
        ...
        self.norm1 = nn.InstanceNorm3d(output_channels, affine=True,track_running_stats=False)
        self.act1 = nn.LeakyReLU(inplace=True)
        
        self.conv2 = nn.Conv3d(output_channels, output_channels, kernel_size, padding=padding)
        self.norm2 = nn.InstanceNorm3d(output_channels, affine=True,track_running_stats=False)
...
#             self.upscale_logits_ops.append(lambda x: x)
    ...
    def forward(self, x):
        ...
        if self._deep_supervision and self.do_ds and False:
